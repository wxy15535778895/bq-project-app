<template>
	<view class="body">
		...
		<view class="top">
			...
			<navigator class="add" url="../Instructions/add">新增</navigator>
		</view>
		<view class="cu-list menu-avatar">
			<view class="cu-item" @click="getId(index)" :class="modalName=='move-box-'+ index?'move-cur':''" v-for="(item,index) in list"
				...
				<view class="content">
					<view class="text-grey">{{item.unit}}</view>
				...
				<view class="move">
					<view class="bg-grey" @tap.stop="getId1(index)">批复</view>
					<view class="bg-grey" @tap.stop="getId1(index)">回执</view>
		...
		methods: {
			...
			getId(index) {
				...
				uni.navigateTo({ //点击跳转的方法
					url: '../Instructions/add?id=' + this.list[index].id //需要跳转的页面路径，使用问号进行id的传值，然后再拼接一个要传入的id
				})
			},
			getId1(index) {
				console.log('111')
				...
				// console.log(this.list[index].id)
				uni.navigateTo({ //点击跳转的方法
					url: '../Instructions/list?id=' + this.list[index].id //需要跳转的页面路径，使用问号进行id的传值，然后再拼接一个要传入的id
				})
			},
			surveyList(page) {
				console.log(page)
