<template>
	<view class="body">
		...
		<view class="top">
			...
			<navigator class="add" url="../gouzaowu/add">添加检查单</navigator>
		</view>
		<!-- 		<view class="neck">
		...
		methods: {
			...
			getId(index) {
				...
				uni.navigateTo({ //点击跳转的方法
					url: '../gouzaowu/list?id=' + this.list[index].id //需要跳转的页面路径，使用问号进行id的传值，然后再拼接一个要传入的id
				})
			},
			surveyList(page) {
				if(page==1){
