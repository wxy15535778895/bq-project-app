	...
	<view class="body">
		...
		<view class="cu-list menu-avatar">
			<view class="cu-item" @click="getId(index)" :class="modalName=='move-box-'+ index?'move-cur':''" v-for="(item,index) in list"
				...
				<view class="content">
					<view class="text-grey">{{item.level}}</view>
		...
		methods: {
			...
			getId(index) {
				...
				uni.navigateTo({ //点击跳转的方法
					url: '../Journal/add?id=' + encodeURIComponent(JSON.stringify(this.list[index])) //需要跳转的页面路径，使用问号进行id的传值，然后再拼接一个要传入的id
				})
			},
			surveyList(page) {
				this.list = []
