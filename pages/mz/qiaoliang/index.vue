...
<!-- 		<view class="top">
			...
			<navigator class="add" url="../qiaoliang/add">添加检查单</navigator>
		</view> -->
		<!-- 		<view class="neck">
		...
		<view class="table" style="margin-top: 18px;padding-bottom: 20%;">
			<table style="width: 100%;" border="1" cellspacing="0" cellpadding="0">
				...
				<tr style="width: 100%;" v-for="(item,index) in list" @click="getId(index)" :key="index">
					<td>{{item.carName}}</td>
					<td><image :src="item.iconLink" style="width: 30px;
    height: 30px;"></image></td>
		...
		methods: {
			...
			getId(index) {
				...
				uni.navigateTo({ //点击跳转的方法
					url: '../qiaoliang/map?id=' + encodeURIComponent(JSON.stringify(this.list[index])) //需要跳转的页面路径，使用问号进行id的传值，然后再拼接一个要传入的id
				})
/* 				console.log(this.list[index])
