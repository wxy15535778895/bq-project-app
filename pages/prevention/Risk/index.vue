<template>
	<view class="body">
		...
		<view class="top">
			...
			<navigator class="add" url="../prevention/Risk/add">添加检查单</navigator>
		</view>
		<!-- 		<view>
		...
		<view class="table" style="margin-top: 18px;padding-bottom: 20%;">
			<table style="width: 100%;" border="1" cellspacing="0" cellpadding="0">
				...
				<tr style="width: 100%;" v-for="(item,index) in list" @click="getId(index)" :key="index">
					<td>{{item.name}}</td>
					<td>{{item.grade==0?'低度风险':item.grade==1?'中度风险':'高度风险'}}</td>
					<td :style="{'background':item.redState==false?'#5FB456':'#FF0100'}" @click.stop="getRedState(index)">{{item.redState==true?'超时':'正常'}}</td>
					<td>{{item.state=="1"?'管控中':item.state=="2"?'观察中':'转为隐患点'}}</td>
				</tr>
		...
		methods: {
			...
			getRedState(index){
				...
				uni.navigateTo({ //点击跳转的方法
					url: '../prevention/Risk/list?id=' + this.list[index].id //需要跳转的页面路径，使用问号进行id的传值，然后再拼接一个要传入的id
				})
			},
			getId(index) {
				console.log(index)
				...
				uni.navigateTo({ //点击跳转的方法
					url: '../prevention/Risk/add?id=' + this.list[index].id //需要跳转的页面路径，使用问号进行id的传值，然后再拼接一个要传入的id
				})
			},
			surveyList(page) {
				if (page == 1) {
				...
				let opts = {
					url: '/securityRiskControlContor/listPage',
					method: 'post'
				};
				console.log(data)
