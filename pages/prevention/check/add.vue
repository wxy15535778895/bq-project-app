		...
		<view class="content">
			...
			<view class="list-1">
				...
				<view class="uni-list-cell-db">
					<picker mode="date" :value="form.time" :start="startDate" :end="endDate" @change="bindDateChange">
			...
			<view class="list-1">
				<view>巡查人员： </view>
				<input type="text" value="" v-model="form.examiner" class="dcinput" />
			</view>
			<view class="list-1">
				<view>巡查区域</view>
				<input type="text" value="" v-model="form.area" class="dcinput" />
			</view>
			<view class="list-1">
				<view>巡查内容</view>
			...
			<view class="list-1">
				<view>巡查结果</view>
				<picker :value="index" @change.prevent.stop="getCouponSelected1" :range="roadDataList1">
					<view ref="addRequestState" class="uni-input">{{form.result?form.result:roadDataList1[index1]}}
						<view class="cu-item" v-for="(item,index) in cuIcon" :key="index" v-if="item.isShow">
							<text class="lg text-gray" :class="'cuIcon-' + item.name"></text>
						</view>
					</view>
			...
			<view class="list-1" v-if="degree">
				<view>危害程度</view>
				<picker :value="index" @change.prevent.stop="getDegree" :range="degreeArray">
					<view ref="addRequestState" class="uni-input">{{degreeArray[index5]}}
						<view class="cu-item" v-for="(item,index) in cuIcon" :key="index" v-if="item.isShow">
							<text class="lg text-gray" :class="'cuIcon-' + item.name"></text>
						</view>
					</view>
			...
			<view class="list-1" v-if="isresult">
				<view>异常类型</view>
				<picker :value="index" @change.prevent.stop="getAbnormalType" :range="abnormal">
					<view ref="addRequestState" class="uni-input">{{form.exceptionType?form.exceptionType==1?"危桥":"危险路段":abnormal[index4]}}
						<view class="cu-item" v-for="(item,index) in cuIcon" :key="index" v-if="item.isShow">
							<text class="lg text-gray" :class="'cuIcon-' + item.name"></text>
						</view>
					</view>
			...
			<view class="list-1" v-if="isbridge">
				<view>坐标</view>
				<input type="text" value="" class="dcinput">{{form.lat}},{{form.lng}}</input>
			</view>
			<view class="list-1">
				<view>备注</view>
...
<script>
	export default {
		data() {
			...
			return {
				...
				form: {
					...
					riskId: '', //风险点
					result: '', //巡查结果
					time: new Date().toISOString().slice(0, 10), //日期, //巡查时间
					examiner: '', //巡查人员
					company: '', //单位名称
					area: '', //巡查区域
					content: '', //巡查内容
		...
		watch: {
			"form.result"(newValue) {
				if (newValue == -1) {
					this.form.result = '请选择'
				}
				if (newValue == 0) {
					this.form.result = '异常'
				}
		...
		onLoad(params) {
			...
			if (JSON.stringify(params) !== '{}') {
				...
				let opts = {
					url: '/securityPatrol/findById',
					method: 'post'
				};
				this.$http.httpRequest(opts, data).then(res => {
					console.log(res)
					this.form = res.data.data
					this.form.checkPictures.forEach(item => {
		...
		methods: {
			...
			getDegree(e) {
				  ...
				  this.form.degree=this.index5-1
				  console.log(this.form.degree)
			},
			//获取异常类型
			getAbnormalType(e) {
			...
			surveyList() {
				...
				uni.getLocation({
					...
					success(res) {
						that.form.lat = res.latitude.toFixed(2)
						that.form.lng = res.longitude.toFixed(2)
			...
			save() {
				...
				if (this.form.result == '异常') {
					this.form.result = 0
				} else if (this.form.result == '正常') {
					this.form.result = 1
				} else {
					this.form.result = null
				}
				if (this.form.type == '日常') {
					this.form.type = 0
				} else if (this.form.type == '专项') {
					this.form.type = 1
				} else {
					this.form.type = null
				}
				console.log(this.form.result, this.form.type)
				...
				} else if (!this.form.type && this.form.type !== 0) {
					uni.showModal({
						title: '提示',
						content: '请选择巡查类型！',
					});
				...
				} else if (!this.form.result && this.form.result !== 0) {
					uni.showModal({
						title: '提示',
						content: '请选择巡查结果！',
					});
				} else if (this.form.result == 0) {
					if (!this.form.exceptionType) {
						uni.showModal({
							title: '提示',
							content: '请选择异常类型！',
						});
