<template>
	<view class="body">
		...
		<view class="top">
			...
			<navigator class="add" url="../prevention/record/add">添加检查单</navigator>
		</view>
		<!-- 		<view>
			当日巡查结果统计:正常{{Inspectionresults.normal}}次 异常{{Inspectionresults.abnormal}}次
		</view> -->
		<!-- 		<view class="neck">
		...
		<view class="table" style="margin-top: 18px;padding-bottom: 20%;">
			<table style="width: 100%;" border="1" cellspacing="0" cellpadding="0">
				...
				<tr style="width: 100%;" v-for="(item,index) in list" @click="getId(index)" :key="index">
					<td>{{item.filingType==2?'隐患查治':'突发应急'}}</td>
					<td>{{item.happenSite}}</td>
					<td>{{item.happenTime.slice(0, 10)}}</td>
					<td :style="{'background':item.revertStatus==1?'#5FB456':'#FF0100'}" @click.stop="getRedState(index,item.revertStatus)">{{item.revertStatus==0?'待处理':'已处理'}}</td>
				</tr>
...
<script>
	...
	export default {
		...
		methods: {
			...
			getRedState(index,type){
				if(type!==1){
				uni.navigateTo({ //点击跳转的方法
					url: '../prevention/record/list?id=' + this.list[index].id //需要跳转的页面路径，使用问号进行id的传值，然后再拼接一个要传入的id
				})	
				}
			},
			getId(index) {
				...
				uni.navigateTo({ //点击跳转的方法
					url: '../prevention/record/add?id=' + this.list[index].id //需要跳转的页面路径，使用问号进行id的传值，然后再拼接一个要传入的id
				})
			},
			surveyList(page) {
				if (page == 1) {
