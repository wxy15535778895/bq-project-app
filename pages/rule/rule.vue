<template>
	<view>
		...
		<uni-collapse :accordion="true">
			<uni-collapse-item open :title="item.title" :show-animation="true" v-for="(item,index) in collData" :key="index">
		...
		<uni-collapse :accordion="true">
			<uni-collapse-item open :title="item.title" :show-animation="true" v-for="(item,index) in collData1" :key="index">
				<view style="padding: 30upx 20upx;" v-for="(item2,index2) in item.content1" :key="index2">
					<view class="con-t">
						{{item2.title}}
					</view>
		...
		data() {
			return {
				...
				collData: [{
					...
					content1: [{
							title: `上班时间`,
							content: `周一,二,三,四,五 全天班次: 09:00-18:00  <br/> 休息时间: 12:30-13:00 <br/> 周六 ， 周日 休息 <br/> `
						},
						{
							title: `迟到与早退`,
							content: `所有班次 <br/> 提前下班视为早退 <br/> `
						},
					],
				}, ],
				collData1: [{
