...
<!-- 					<view class="uni-list-cell">
						...
						<view class="uni-list-cell-db">
											...
											<picker mode="date" :value="time" :start="startDate" :end="endDate"
			...
			<navigator class="add" url="../security/add">新增</navigator>
		</view>
		<view class="cu-list menu-avatar">
			<view class="cu-item" @click="getId(index)" :class="modalName=='move-box-'+ index?'move-cur':''" v-for="(item,index) in list"
				...
				<view class="action" style="width: 70px;">
					<view class="text-grey text-xs">{{item.createTime.slice(0, 10)}}</view>
					<!-- <view class="cu-tag round bg-grey sm">5</view> -->
					{{item.remark}}
				</view>
				<view class="move">
					<view class="bg-grey" @click="getId(index)">回执</view>
		...
		methods: {
			...
			getId(index) {
				...
				uni.navigateTo({ //点击跳转的方法
					url: '../security/add?id=' + this.list[index].id //需要跳转的页面路径，使用问号进行id的传值，然后再拼接一个要传入的id
				})
			},
			surveyList(page) {
				console.log(page)
				...
				let data = {
					...
					// createTime: this.patrolTime,
					rankId:"",
					type:'安全管理'
				};
				let opts = {
					url: '/documentManagement/listPage',
					method: 'post'
				};
				console.log(data)
				this.$http.httpRequest(opts, data).then(res => {
