	...
	<view class="body">
		...
		<view class="top">
			...
			<!-- <view class="search">搜索</view> -->
				<navigator class="add"  url="../suidao/add">添加检查单</navigator>
		</view>
<!-- 		<view class="neck">
...
白河管理所
		...
		<view class="table" style="margin-top: 18px;padding-bottom: 20%;">
			<table style="width: 100%;" border="1" cellspacing="0" cellpadding="0">
				...
				<tr v-if="item.tunnel" style="width: 100%;" v-for="(item,index) in list" @click="getId(index)" :key="index">
					<td>{{item.tunnel.stakeMarkCenter}}</td>
					<td>{{item.tunnel.name}}</td>
				...
				uni.request({
					...
					success: (res) => {
						...
						res.data.data.forEach(item => {
							console.log(item.id)
							if(item.id==this.list[index].tunnelId){
								arr.push(item)
							}
						})
						let _self = this;
						...
						uni.setStorage({
							...
							success: function() {
								...
								uni.navigateTo({
									url: '../suidao/list?id=' + _self.list[index].id //需要跳转的页面路径，使用问号进行id的传值，然后再拼接一个要传入的id
								})
							}
						})
					}
