<template>
	<view class="body">
		...
		<view class="top">
			...
			<navigator class="add" url="../supervisor/add">新增</navigator>
		</view>
		<view class="cu-list menu-avatar">
			<view class="cu-item" @click="getId(index)" :class="modalName=='move-box-'+ index?'move-cur':''" v-for="(item,index) in list"
				...
				<view class="action" style="width: 70px;">
					<view class="text-grey text-xs">{{item.workTime}}</view>
		...
		methods: {
			...
			getId(index) {
				...
				uni.navigateTo({ //点击跳转的方法
					url: '../supervisor/add?id=' + this.list[index].id //需要跳转的页面路径，使用问号进行id的传值，然后再拼接一个要传入的id
				})
			},
			surveyList(page) {
				console.log(page)
